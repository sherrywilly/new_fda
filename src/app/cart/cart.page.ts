...
async applyCoupen(id)
{
  const loading = await this.loadingController.create({
    ...
    this.server.applyCoupen(id,localStorage.getItem('cart_no')+"?lid="+localStorage.getItem('lid')+"&lat="+localStorage.getItem('current_lat')+"&lng="+localStorage.getItem('current_lng')).subscribe((response:any) => {
